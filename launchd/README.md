...
| Run | Why |
|---|---|
| **05:45** | Captures the prior day's completed games. Late enough that upstream stat sources (Fangraphs / Savant) have settled overnight — earlier fires landed mid-update and the first morning login saw stale numbers. Still ahead of the 09:00 waiver claim window, so the day's target list is ready well before claims fire. |
...
Why LaunchAgent over `cron`: macOS `cron` does not fire missed jobs when the Mac is asleep at the scheduled time — the run is silently skipped. `launchd` instead fires on wake: a job whose scheduled time passed while the Mac slept runs as soon as it wakes. Paired with `pmset` we also wake the Mac just before the **05:45** fire, so the early-morning run is reliable even when the laptop is closed. The 09:05 run relies on the Mac being awake (typical for a workday morning) or on launchd's fire-on-wake catch-up — see the `pmset` note below.
...
Use this to fire the pipeline immediately instead of waiting for the next scheduled run (05:45 or 09:05). It's the single most useful command in this whole setup — for verification after install, dry-running code changes, and re-running after a fixed failure.
...
- **Does not change the schedule** — the next scheduled fire (05:45 / 09:05) still happens as configured
...
1. **Keep Prefect on its schedule** — 05:45 + 09:05 (this LaunchAgent, already installed).
2. **Schedule the bash runner at a non-conflicting time**, e.g. 06:30 MST — between the two Prefect runs and ~45 min after the 05:45 fire so Prefect has finished by then. The bash runner writes to the same Postgres + Neon target, so running it AFTER the 05:45 Prefect run lets you compare "did the bash version land the same data as Prefect did ~45 min earlier".
3. **Manual or scripted daily diff** — quick options:
    - Compare row counts in Neon: `SELECT COUNT(*) FROM players;` (etc.) right after each run completes (~06:00 for Prefect, ~06:45 for bash)
...
**05:45 run does not fire** — first check `pmset -g log | tail -50` to see whether the Mac actually woke at 05:43 or went deeper into sleep. If wake didn't happen, `pmset` wasn't set or got reset by a macOS update. Re-run the `sudo pmset repeat wakeorpoweron MTWRFSU 05:43:00` command.

**05:45 run fires but exits with status 2** — that's the Docker preflight giving up. The plist runs `open -ga Docker` and then polls `docker info` every 2s for 120s before falling through to the pipeline; exit 2 means Docker Desktop never came up in time. Common causes: (1) Docker Desktop is mid-update and the daemon is stuck; (2) "Use Rosetta for x86_64/amd64 emulation" got toggled and Docker is in a recovery loop; (3) the disk is full and Docker can't start its VM. Open Docker Desktop manually, watch it settle, then re-fire with `launchctl kickstart -k gui/$(id -u)/com.mtbl.prefect.nightly`. If exit 2 recurs, raise the poll budget (`seq 1 60` → `seq 1 120`) for a 240s ceiling.

**05:45 run fires but exits with status 1 and the err log ends in `dial unix … docker.sock: no such file or directory`** — the preflight succeeded but Docker died between the poll and the `docker compose run`. Race is rare on a stable host; if you see it repeatedly, the Mac is probably re-sleeping mid-run (System Settings → Battery / Energy → "Wake for network access" or "Prevent automatic sleeping when display is off").
